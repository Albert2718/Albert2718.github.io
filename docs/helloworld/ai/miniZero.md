...
介绍完了核心的一些算法，最后来谈谈黑白棋吧。

在AlphaGo Zero的训练中，其可以一步步学习到一些特殊的围棋技巧或者一些定式：
![alt text](images/image-4.png)

而黑白棋中的奥秘也有很多，可以参考知乎问题：[玩黑白棋有什么技巧？](https://www.zhihu.com/question/25271618)

简要来说：

- 棋盘的四个角落很重要，所以说，角落邻近的位置很危险，一旦在这些位置落子，往往会被对手占据角落。  
- 尽量在游戏初期减少自己的棋子数量，尽量被对手的棋子围在中间，这点和围棋刚好相反  

我自己也和AI下了很多局，发现像我一样的新手在前期下黑白棋很容易陷入贪心，总想着一步可以吃掉多少子，就算明白一些位置的危险性，但在剩下的可以行动的位置中，还是想着多吃子，实则，黑白棋是一款很看重全局观的游戏，注重对局面的控制，前期的布局目的是为了让自己有更多的棋可下，而不是能吃掉多少棋。这在黑白棋中被称作**行动力**：

![alt text](images/image-5.png)
![alt text](images/image-6.png)

>黑棋：白棋=1：12，绝对劣势。
>**在尾盘之前，对黑白棋局势的判断，应基于双方行动力的多少，而不是子数。偏激一点说，黑白棋的争夺，就是对行动力的争夺。**

所以可以看看NN和普通MCTS对弈的结果，两边都是模拟1000次：

![alt text](images/benchmark_results.png)
...
耗时统计:
  NN 总耗时:   421.6s (平均 42.2s/局)
  MCTS 总耗时: 738.8s (平均 73.9s/局)

角落占据统计:
  NN 占据角落:   30 个 (平均 3.00/局)
  MCTS 占据角落: 10 个 (平均 1.00/局)
```

不可思议，我们或许不知道他究竟理解了什么，但NN明显学会了很多黑白棋的精髓。
